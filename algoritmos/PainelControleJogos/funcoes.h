...
void Login(int *plogado)
{
     ...
     system ("cls");
     //printf("aqui sou isso: %d",*plogado);
     //Entrada de Dados
     do
     {
     printf ("Usuario: ");
    ...
    {
        while ((!feof(arq_jogadores)) && senhaok == 0)
 	    {
		 fread(&BuscaJogador,sizeof(JOGADOR),1,arq_jogadores);
                ...
                if (jogo.cod == opcaojogo)
                   {
                    ...
                    //Abre jogo
                    if (opcaojogo == 1)
                       system("e:\\PainelControleJogos\\jogos\\forca\\winforca.exe");
                    else if (opcaojogo == 2)
                       system("e:\\PainelControleJogos\\jogos\\musical\\musical.exe");  
...
void AtualizaRanking(char nome_jogador[MAXNOMEJOGADOR], char nome_jogo[MAXNOMEJOGO], int resultado)
{
     char linha[255], novalinha[255],jogo_arquivo[MAXNOMEJOGO], ponto_arquivo[3], resultado_char[3], *VetorLinhas[5];
     int i=0, j=0,x=0;
     ...
     else
     {
         ...
         //agora agente vai atualizar a tosquera
         if((arq_rank = fopen("ranking.txt", "r"))==NULL)
             printf("erro de abertura do arquivo ranking.txt\n");
         else
         {
         ...
         if((arq_rank = fopen("ranking.txt", "w+"))==NULL)
             printf("erro de abertura do arquivo ranking.txt\n");
         else
         {
              //printando noarquivo
              for (x=0;x<strlen(VetorLinhas);x++)
                if (VetorLinhas[x]!=NULL)
                   fprintf(arq_rank,"%s",VetorLinhas[x]);
                //printf("%s",VetorLinhas[x]);
                //fputs (VetorLinhas[x],arq_rank);
              fclose(arq_rank);
